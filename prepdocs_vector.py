...
print(f"AZURE_SEARCH_ADMIN_KEY: [{search_key}]") 
...
def create_search_index():
    if verbose: print(f"Ensuring search index {index_name} exists")
    ...
    if index_name not in index_client.list_index_names():
        ...
        if verbose: print(f"Creating {index_name} search index")
...
def remove_from_index(filename):
    if verbose: print(f"Removing sections from '{filename or '<all>'}' from search index '{index_name}'")
    search_client = SearchClient(endpoint=f"https://{searchservice}.search.windows.net/",
                                    index_name=index_name,
